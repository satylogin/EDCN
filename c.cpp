...
void encode(char buffer[], int len, char file[], map<string, int> &code, map<int, string> &icode)
{
	int i, next_free;
	string pre;
	...
	for (map<string, int> :: iterator it = code.begin(); it != code.end(); ++it) {
...
int main()
{
	...
	map <string, int> code;
	map <int, string> icode;
	encode(buffer, len, encoded_file, code, icode);
